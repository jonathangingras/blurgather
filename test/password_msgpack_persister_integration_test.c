...
sweetgreen_teardown(persister_integration_test) {
  bg_pwd_repository_destroy(&persister.repository);
}
...
sweetgreen_test_define(persister_integration_test, canSerializeContainedPasswords) {
  bg_password* password = bg_password_init(NULL, &dummy_iv_init, &encryptor, &decryptor, &persister.repository);
  ...
  bg_pwd_repository_add(&persister.repository, password);

  remove("shadow");
  sweetgreen_expect_true(access("shadow", F_OK) == -1 );
        
  bg_pwd_repository_persist(&persister.repository);
...
sweetgreen_test_define(persister_integration_test, canDeserializeContainedPasswords) {
  bg_password_msgpack_persister persister2;
  bg_password_factory factory2;
  bg_password_msgpack_persister_init(&persister2, "shadow", &factory2);
  bg_password_factory_init(&factory2, &dummy_iv_init, &persister2.repository, &encryptor, &decryptor);

  sweetgreen_expect_equal(0, persister2.number_passwords);
  sweetgreen_expect_equal(0, bg_pwd_repository_load(&persister2.repository));
  sweetgreen_expect_equal(1, persister2.number_passwords);

  bg_pwd_repository_destroy(&persister2.repository);
}
...
sweetgreen_test_define(persister_integration_test, deserializationIsDeserializingGoodValue) {
  ...
  sweetgreen_expect_equal_memory(SAVED_IV_1, bg_password_iv_value(persister2.password_array[0]), DUMMY_IV_LENGTH);
  sweetgreen_expect_equal_string(SAVED_NAME_1, bg_password_name(persister2.password_array[0]));
  sweetgreen_expect_equal_string(SAVED_DESCRIPTION_1, bg_password_description(persister2.password_array[0]));
  ...
  sweetgreen_expect_equal_memory(passvalue, bg_password_value(persister2.password_array[0]), BLURGATHER_PWD_MAX_VALUE_LEN);
...
sweetgreen_test_define(persister_integration_test, canSerialize2ContainedPasswords) {
  ...
  bg_password* password2 = bg_password_init(NULL, &dummy_iv_init, &encryptor, &decryptor, &persister.repository);
  ...
  bg_pwd_repository_add(&persister.repository, password2);
...
sweetgreen_test_define(persister_integration_test, canDeserializeContained2Passwords) {
  ...
  sweetgreen_expect_zero(persister2.number_passwords);
  sweetgreen_expect_zero(bg_pwd_repository_load(&persister2.repository));
  sweetgreen_expect_equal(2, persister2.number_passwords);
...
sweetgreen_test_define(persister_integration_test, deserializationIsDeserializingGoodValueFor2Passwords) {
  ...
  sweetgreen_expect_equal_memory(SAVED_IV_2, bg_password_iv_value(persister2.password_array[1]), DUMMY_IV_LENGTH);
  sweetgreen_expect_equal_string(SAVED_NAME_2, bg_password_name(persister2.password_array[1]));
  sweetgreen_expect_equal_string(SAVED_DESCRIPTION_2, bg_password_description(persister2.password_array[1]));
  ...
  sweetgreen_expect_equal_memory(passvalue2, bg_password_value(persister2.password_array[1]), BLURGATHER_PWD_MAX_VALUE_LEN);
